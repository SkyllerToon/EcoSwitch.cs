...
public partial class PlayerFrog : CharacterBody2D
{
    // === Constants ===
    private const float FROG_GRAVITY = 12f;
    private const float FROG_SPEED = 100f;
    private const float FROG_JUMP = -240f;
    private const float FROG_SWIM = 60f;
    private const float FROG_SWIM_JUMP = -120f;

    // === Variables ===
    private float speed = FROG_SPEED;
    private float jump = FROG_JUMP;
    ...
    private float knockPower = 22f;
    private string state = "Frog";
    ...
    private void ApplyGravity()
    {
        if (!IsOnFloor())
        {
            Velocity += new Vector2(0, FROG_GRAVITY);
        }
        else
        {
            fall = false;
    ...
    private void OnHurtboxBodyEntered(Node body)
    {
        ...
        {
            Vector2 kb = new Vector2((GlobalPosition.X - node2D.GlobalPosition.X) * knockPower, -220);
